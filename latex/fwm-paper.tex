...
linearly polarised (LP) modes of a multimode optical fiber \cite{heskethNonlinearEffectsMultimode2014}; two-wave and three-wave mixing in quadratic nonlinear media, polarisation dynamics in nonlinear fibres, and parity-time symmetric nonlinear couplers \cite{heskethGeneralComplexEnvelope2015}; 
...
This invariance property, which to the author's knowledge has not been previously reported for FWM, suggests deep underlying structure that may extend to other parametric processes.
...
The analytic solutions are plotted using using \eqref{eq:u-v-quartic} to calculate $u_j, v_j$, then converting to $A_j, A_j^{*}$ via \eqref{eq:u-v-A} (dashed lines), and are compared against numerical integration of the original physical FWM system \eqref{eq:fwm-system} (symbols).
...
The code used in this paper is available at \cite{heskethHeskethGDFwmpaper2026} and includes Python Jupyter notebooks with symbolic derivations using \emph{SymPy} \cite{meurerSymPySymbolicComputing2017} as well as the code for numeric validation used in Section \ref{sec:num-val}.

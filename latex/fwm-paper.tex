...
\phi_j^{NL}(z)  &= \sum_{k=1}^{4} \left({a}_{j,k} - \frac{1}{4}\sum_{l=1}^{4} {a}_{l,k}\right) \int_{0}^{z} \hat{u}_k (\xi) \hat{v}_k (\xi) \, d\xi \\
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\captionof{table}{Initial field amplitudes used in Figure \ref{fig:case2-plot1}.}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\captionof{table}{Physical parameters used in Figure \ref{fig:case2-plot1}.}
...
\noindent
The $\beta_j$ values listed above had their large mean removed to avoid plotting rapid phase oscillations.
\printbibliography

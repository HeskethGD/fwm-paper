...
We now transform \eqref{eq:drho-sqrd-4} from quartic to the standard cubic form of the Weierstra\ss{} $\wp$ function using the classical trick which can be conceptualised in three steps:
\begin{align}
...
w\left(z\right) &= C_1\,s\left(z\right) + C_0 & \text{shift and scale with $C_j$ to match Weierstra\ss{} coefficients}. \label{eq:wp-4-to-3}
\end{align}
...
As $\wp$ is an even function, it is neccessary to also specify a corresponding condition for the derivative when inverting, i.e., to find $z$ from known $x,y$ we give conditions such as $\wp\left(z\right)=x, \wp'\left(z\right)=y$, where $\wp'$ is the derivative of $\wp$ known as Weierstra\ss{} p-prime. 
...
We substitute \eqref{eq:uv-wp} into \eqref{eq:dlog-u-v-rho} and use the Weierstra\ss{} elliptic function identity:
\begin{equation}
...
In this section we show that the conservation of the Hamiltonian can be viewed as a manifestation of the Frobenius-Stickelberger determinant formula.
The starting point is the following relation established via \eqref{eq:ham-uv}, \eqref{eq:duv_j}, and \eqref{eq:Q-uv}:
...
where recall $\gamma_j=\rho(\mu_j)$ and the sum over $b_l \gamma_j$ can be written using \eqref{eq:drho-sqrd-2} as:
\begin{align}
...
\section{Plotting analytic vs numeric solution}

To check the validity of the solutions, we evaluate the analytic solutions using Sympy in Python and we plot them against numeric solutions of the corresponding system of differential equations founds using scipy in Python.
...
The analytic dashed lines are shown to be in agreement with the numeric solutions shown as symbols. 
As expected in nonlinear fiber optic four-wave mixing, periodic intermodal power exchnage is seen between modes in Figure \ref{fig:A-abs}, 

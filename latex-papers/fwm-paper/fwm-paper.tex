...
The procedure sketched in \eqref{eq:wp-4-to-3} is implemented in one go using the following transformation:
\begin{align}
...
The equation \eqref{eq:dwp} is the defining differential equation for the Weierstra\ss{} elliptic $\wp$ function and the solution is:
\begin{equation}
...
The modal powers can then be expressed in terms of the constants as follows:
\begin{align}
u_j{\left(z\right)} v_j{\left(z\right)} &= \frac{\wp'{\left(z_{1}\right)}}{ \sqrt{{d}_{4}} \left(\wp{\left(z_{1}\right)} - \wp{\left({\mu}_{j} - z_{0}\right)}\right)} - \frac{\wp'{\left(z_{1}\right)}}{\sqrt{{d}_{4}} \left(\wp{\left(z_{1}\right)} - \wp{\left(z - z_{0}\right)}\right) }, \\
&= \frac{\wp'{\left(z_{1}\right)} \left(\wp{\left(z - z_{0}\right)} - \wp{\left({\mu}_{j} - z_{0}\right)}\right) }{ \sqrt{{d}_{4}} \left(\wp{\left(z_{1}\right)} - \wp{\left({\mu}_{j}- z_{0}\right)}\right) \left(\wp{\left(z - z_{0}\right)} - \wp{\left(z_{1}\right)}\right) }
\end{align}

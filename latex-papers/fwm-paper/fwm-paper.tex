...
In \cite{chenFourphotonParametricMixing1989, chenFourwaveMixingOptical1989} solutions for the power in each of the frequency modes is derived by inverting elliptic integrals to obtain elliptic functions.
...
This is the first of our two main results.
...
This is the second of our main results.
...
In this section we recall the four-wave mixing system that forms the focus of our study. 
...
The system describes four-wave mixing in the quasi continuous wave limit, i.e., in the absence of time derivatives:
...
\begin{itemize}
    ...
    \item $\beta_j = n(\omega_j)\omega_j/c$ are the propagation constants (also referred to as phase velocities)
\end{itemize}
...
Agrawal says of \eqref{eq:fwm-system} that the equations ``are quite general in the sense that they include the effects of SPM, XPM, and pump depletion on the FWM process''.
Herein, they are solved analytically in full, as written, without further approximation.
...
In this section we normalise the system parameters to make the forthcoming algebra manageable. 
...
\end{align}
In all that follows we will refer to $u_j, v_j$ as modes and we use this as a general term to describe components of an abstract coupled system and it should not be confused with propagation modes of a multimode optical fibers.
...
In order to obtain elliptic function solutions in wave mixing dynamics, one key thing to note is that the derivative of the modal power in \eqref{eq:duv_j} is proportional to the difference of the two wave mixing product terms, and that the Hamiltonian in \eqref{eq:ham-uv} contains their sum.
...
The solutions in \eqref{eq:u-v-bar} are, up to constant and $z$ dependent exponential factors, Kronecker theta functions, which simply put are a ratio of two Weierstrass $\sigma$ functions shifted relative to each other in their argument.
...
In this final section we check the validity of the solutions by evaluating them numerically, whilst also demonstrating that we can do so using open source software in Python. 
We evaluate the analytic solutions numerically using the \emph{pyweierstrass} Python package \cite{stephanePyweierstrass2022} which provide Weierstrass elliptic functions as a wrapper around Jacobi elliptic functions from the \emph{mpmath} Python package \cite{MpmathPythonLibrary2023}. 
We plot them against numeric solutions of the corresponding system of differential equations found using the DOP853 Runge-Kutta algorithm from \emph{SciPy} in Python \cite{virtanenSciPy10Fundamental2020}.

In Figures \ref{fig:u-v-re} and \ref{fig:u-v-im} we plot the real and imaginary parts, respectively, of the product of $u_jv_j$ in an abstract non-physical $u,v$ scenario using Equation~\eqref{eq:uv-wp}.

...
It underpins a wide range of applications, including wavelength conversion, parametric amplification, frequency-comb generation, and quantum light sources (see \cite{agrawalNonlinearFiberOptics2019} and references therein). 
...
Two notable works have previously presented relevant analytic solutions for four-wave mixing. 
In \cite{chenFourwaveMixingOptical1989} solutions for the power in each of the frequency modes is derived by inverting elliptic integrals to obtain elliptic functions.
In \cite{marhicAnalyticSolutionsPhases2013} elliptic integrals are given for the phases for each mode. Both works use Jacobi elliptic functions. 
In contrast, this work derives complete solutions for the complex mode envelopes themselves, avoiding decomposition into amplitude and phase or real and imaginary parts, and does so using Weierstrass elliptic functions.
...
The emergence of these functions establishes a direct connection between four-wave mixing and other nonlinear optical systems \cite{heskethGeneralComplexEnvelope2015}. 
...
The following coupled system of ordinary differential equations is taken from \cite{agrawalNonlinearFiberOptics2019} which credits \cite{stolenParametricAmplificationFrequency1982} with its creation. 
The system describes four-wave mixing in the continuous wave limit, i.e., in the absence of time derivatives:
...
The fundamental Weierstrass elliptic function theory used in coming sections can be found in \cite{whittakerCourseModernAnalysis2021}. 
One key thing to note in order to obtain elliptic functions solutions in wave mixing dynamics is that the derivative of the modal power in \eqref{eq:duv_j} is proportional to the difference of the two wave mixing product terms, and that the Hamiltonian in \eqref{eq:ham-uv} contains their sum.
...
After choosing a root, $\lambda_1$, the procedure sketched in \eqref{eq:wp-4-to-3} is implemented in the following single transformation:
\begin{align}
...
\begin{itemize}
    ...
    \item the differential equations structure in regards to recognisable four-wave mixing terms                                                                                                                      
\end{itemize}
...
Next, we show how we can perform a coordinate transformation that involves a local, i.e. $z$ dependent, normalisation of the modes themselves and, at the same time, induces a Mobius transform on modal powers which implements the classic trick that reduces the quartic to the cubic discussed in Section~\ref{sec:mode-power}.
However, here that same effect is delivered at the level of modes $u_j, v_j$ not only at the mode power level $u_j v_j$ so that we can observe a corresponding effect on the coupled system.
...
\end{align}
where $\theta_j$ and $d_5$ are given in Appendix \ref{app:param-def-appendix}. 
The $\theta_j$ parameter is not involved in the reduction of the quartic to the cubic but its inclusion keeps the phase velocity coefficient the same for all modes after the transform.
The key parameter in the transformation is the root $\lambda_1$ previously defined for the purpose of quartic to cubic reduction.

The transformation in \eqref{eq:hat-to-bar} sends \eqref{eq:uv-hat-system} to:
\begin{align}
\frac{\partial}{\partial z} \bar{u}_j &= -\left(\frac{1}{\chi} - \frac{\chi}{4}\bar{u}_j \bar{v}_j \right) \bar{u}_j - \frac{\chi}{4} \prod\limits_{k=1, k \ne j}^{4} \bar{v}_k, \notag \\
\frac{\partial}{\partial z} \bar{v}_j &= \left( \frac{1}{\chi} - \frac{\chi}{4}\bar{u}_j \bar{v}_j\right) \bar{v}_j + \frac{\chi}{4} \prod\limits_{k=1, k \ne j}^{4} \bar{u}_k \label{eq:uv-bar-system}
\end{align}
...
\end{align}
and the intermodal power conservation laws are:
\begin{align}
\bar{u}_j(z) \bar{v}_j(z) - \bar{u}_k(z) \bar{v}_k(z) = \frac{{d}_{5}}{4 \left({\gamma}_{j} - {\lambda}_{1}\right)} - \frac{{d}_{5}}{4 \left({\gamma}_{k} - {\lambda}_{1}\right)}
\end{align}

The modal power in these coordinates evolves according to:
\begin{equation}
\label{eq:duv_bar_j} 
\frac{d}{d z} \bar{u}_j \bar{v}_j = \frac{\chi}{4}\left(\prod\limits_{k=1}^{4} \bar{v}_k - \prod\limits_{k=1}^{4} \bar{u}_k\right)
\end{equation}
...
We see that as a result of the transformation in \eqref{eq:hat-to-bar}, the right hand side of \eqref{eq:dwp-bar} is cubic in $\bar{w}$ and thus any differential equation in $h$ or $\bar{u}_j\bar{v}_j$ is also cubic in those functions as they are linearly related.
...
The solutions in \eqref{eq:u-v-bar} are Kronecker theta functions (up to constant and $z$ dependent exponential factors), which simply put are a ratio of two Weierstrass $\sigma$ functions shifted relative to each other in $z$.
This form is similar to that obtained in other systems in nonlinear optics such as two and three wave mixing in quadratic nonlinear media, polarisation dynamics in nonlinear fibers, and parity-time symmetric nonlinear couplers \cite{heskethGeneralComplexEnvelope2015}. 
...
\begin{align}
\bar{u}_j(z) &= -\frac{i}{\chi}\tilde{u}_j\left(\frac{z}{\chi}\right) \\
\bar{v}_j(z) &= \frac{i}{\chi}\tilde{v}_j\left(\frac{z}{\chi}\right) \\
z &= \chi\xi
\end{align}
...
\tilde{v}_j{\left(\xi\right)} &= \frac{ \sigma{\left(\xi - {\tilde{\mu}}_{j}\right)} \exp\left({-\xi \left( 1 + \frac{{\tilde{\gamma}}_{j}}{2} - \zeta{\left( {\tilde{\mu}}_{j} - \xi_{0}\right)}\right)}\right)}{\tilde{\epsilon}_{j} \,\sigma{\left(\xi - \xi_{0}\right)} \sigma{\left( {\tilde{\mu}}_{j} - \xi_{0}\right)}}
\end{align}
...
\section{Conclusion}
We have derived a complete analytic solution to the continuous-wave four-wave mixing system. 
The solutions are expressed in terms of Weierstrass elliptic functions, and we have demonstrated how they may be evaluated numerically using open-source Python code. 
Motivated by the structure of these analytic solutions, we investigated a class of coordinate transformations that yield a canonical coordinate system in which the governing differential equations simplify substantially and the solutions become single-valued, taking the form of Kronecker theta functions.

Through this exploration of coordinate transformations, we showed that the full four-wave mixing system can be reduced to a significantly simpler, parameter-free system. 
...
Four-wave mixing provides an important and challenging test case for mathematical analysis, and is arguably among the more intricate examples of dynamical systems admitting elliptic (genus-one) function solutions due to the presence of quartic polynomials. 
In this work, we have demonstrated in detail how these difficulties can be systematically addressed. 
...
In forthcoming work, we intend to present a unified framework for integrable nonlinear coupled ordinary differential equations with elliptic and hyperelliptic function solutions, for which the four-wave mixing system studied here will serve as a central example.
...
\bar{\kappa}_j &= - \zeta{\left(- z_{0} + {\mu}_{j}, g_2, g_3\right)} - {b}_{2} {\gamma}_{j} + 2 {\theta}_{j} + \frac{1}{\chi}
\end{align}
In equating the two equivalent definitions of $\chi$ we have chosen the positive branch of the square root when evaluating \eqref{eq:drho-sqrd-2} at $\rho(z)=\lambda_1$.
...
\begin{align}
\tilde{H} &= \chi^3\bar{H}, \\
\tilde{\epsilon}_j &= \epsilon_j e^{\frac{i\pi m}{4}}, \notag \\
